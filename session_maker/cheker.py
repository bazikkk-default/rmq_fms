...
ErrorType = Dict[int, Union[str, List[str]]]


def check_status_code(response_status: int = 200):
    try:
        assert response_status == 200, "Something went wrong"
    except AssertionError:
        raise StatusCodeError(error=went_wrong(response_status))


def went_wrong(response_status: int = None) -> ErrorType:
    codes = {
        400: [
            'Bad Request',
    ...
    return {response_status: codes.get(response_status, 'Unknown error')}
